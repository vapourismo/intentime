...
- Notifications use floating `NSPanel` (HUD style) banners instead of `UNUserNotificationCenter` because SwiftPM executables lack a bundle identifier. Break-start banners auto-dismiss after 4 seconds; break-end prompts persist until the user responds. Break-start banner body copy and break-end prompt body copy are randomized from pools of 20 cheeky variants each while preserving the original intent
...
- Menu bar title behavior is unchanged for non-break contexts: `MM:SS - message` (timer with message), `MM:SS` (timer only), or `message` (message only). During active short/long breaks, the message portion is replaced by randomized break-encouragement copy (picked from phase-specific pools and kept stable for that break). Break encouragement lines are action-oriented, witty, and do not use periods. During work, the icon is a custom-drawn progress circle (pie chart filling clockwise from 12 o'clock). During breaks, the icon is `cup.and.saucer.fill`. When paused, the icon changes to `pause.fill`
...
- App logo source is generated (not hand-drawn in external tools): `scripts/generate_icon.swift` renders a stopwatch icon template and `scripts/generate-icon.sh` scales it into an `.iconset` and compiles `Resources/AppIcon.icns` via `iconutil`
