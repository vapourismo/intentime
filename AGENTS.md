...
- Timer uses `UserDefaults` to persist `endTime` (epoch seconds); remaining time is computed on each tick using `ceil(endTime - now)` so visible seconds do not skip when callbacks drift slightly (for example right after resume/unpause). Paused state is persisted as `pausedMillisecondsLeft` (integer milliseconds) with legacy migration from `pausedSecondsLeft`; on pause the end time is cleared and remaining milliseconds saved; on unpause a new end time is computed
...
- Timer can be paused and resumed during work phases; paused state survives app restart via `UserDefaults`. Any active countdown also auto-pauses when macOS locks the session (`com.apple.screenIsLocked`); if this happens during a break, the blur overlay is temporarily dismissed and restored on resume. Users can skip any phase ("Go to Break" or "End Break")
...
- System-wide hotkey (Cmd+Shift+Space) opens a Spotlight-like HUD panel for setting the focus message. Uses Carbon `RegisterEventHotKey` (no Accessibility permissions needed, works without a bundle identifier). The same panel is used by the "Set Message" / "Edit Message" menu item, and those menu actions display `⌘⇧Space` as their shortcut hint. Enter confirms, Escape cancels, pressing the hotkey again toggles the panel closed. The message input is strictly single-line: long text does not wrap, and the panel width auto-expands/shrinks while typing (minimum 400 px, up to 900 px or 85% of visible screen width) before falling back to horizontal scrolling
