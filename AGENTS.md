...
- Menu bar title behavior is unchanged for non-break contexts: `MM:SS - message` (timer with message), `MM:SS` (timer only), or `message` (message only). During active short/long breaks, the message portion is replaced by randomized break-encouragement copy (picked from phase-specific pools and kept stable for that break). During work, the icon is a custom-drawn progress circle (pie chart filling clockwise from 12 o'clock). During breaks, the icon is `cup.and.saucer.fill`. When paused, the icon changes to `pause.fill`
