...
- Settings are accessible via a "Settings…" menu item (⌘,). The settings panel is a floating HUD `NSPanel` with number fields for work duration, short break, long break, extend break (all in minutes), sessions before long break. Changes take effect on the next phase (the currently running phase keeps its original duration)
